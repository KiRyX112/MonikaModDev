...
label mas_gender:
    ...
    menu:
        ...
        "Парень.":
            ...
            m 1hksdlb "Не так много девушек стали бы играть в эту игру, {do_giggle}э-хе-хе~"
        "Девушка.":

            $ persistent.gender = "F"
            m 2eud "О? Так ты на самом деле девушка?"
            m 2hksdlb "Надеюсь, раньше я ничего не говорила, что могло бы обидеть тебя!"
            m 7rksdlb "...Наверное, поэтому и говорят, что не стоит делать предположений, {do_giggle}а-ха-ха!"
...
label mas_birthdate:
    ...
    if persistent._mas_player_bday is not None:
            ...
            "Да.":
                if datetime.date.today().year - persistent._mas_player_bday.year < 5:
                    ...
                    if not mas_isplayer_bday():
                        ...
                        m 3hksdlb "Мне просто надо было убедиться, просто не хотелось бы понять что-то важное, как твоя дата рождения, неправильно, {do_giggle}а-ха-ха!"
        ...
        m 3hub "А это именно то, что я должна знать, {do_giggle}а-ха-ха!"
...
label birthdate_set:
    ...
    if mas_isplayer_bday() and not mas_isMonikaBirthday():
        ...
        if old_bday == mas_player_bday_curr():
            if mas_isMoniNormal(higher=True):
                m 3hub "{do_giggle}А-ха-ха! Твой день рождения, оказывается, {i}уже{/i} настал!"
                m 1tsu "Я рада, что уже подготовилась к нему, {do_giggle}э-хе-хе..."
            ...
            if mas_isMoniNormal(higher=True):
                ...
                m 1hub "{do_giggle}А-ха-ха! Это не так много, но хоть что-то!"
    ...
    # have to use the raw data here to properly compare in the rare even that the player bday and first sesh are on 2/29
        ...
        m 1tsu "Похоже, нам и вправду суждено быть вместе, {do_giggle}э-хе-хе..."
        ...
        m 3hub "Столько сладостей в один день, {do_giggle}а-ха-ха!"
...
label calendar_birthdate:
    ...
    m 3hub "Твой день рождения, {do_giggle}а-ха-ха!"
...
label mas_unlock_hangman:
    ...
    if mas_safeToRefDokis():
        ...
        if persistent.playername.lower() == "сайори":
            m 3tku "...Возможно~"
...
label mas_unlock_piano:
    ...
    if not persistent._mas_pm_plays_instrument:
        ...
        m 4hub "Так что я ожидаю чего-то грандиозного! {do_giggle}Э-хе-хе~"
    ...
    if persistent._mas_idle_data.get("monika_idle_game", False):
        ...
        menu:
            ...
            "Нет.":
                ...
                m 3hub "Что ж, надеюсь, тебе было весело до того, как произошёл краш, {do_giggle}а-ха-ха!"
                if mas_isMoniHappy(higher=True):
                    m 1hubsa "Я просто рада, что ты вернулся ко мне~"
        m 2rksdla "Но всё же..."
    if renpy.android:
...
label mas_bookmarks_notifs_intro:
    if not renpy.seen_label('bookmark_derand_intro') and (len(persistent._mas_player_derandomed) == 0 or len(persistent._mas_player_bookmarked) == 0):
        ...
        if len(persistent._mas_player_derandomed) == 0 and len(persistent._mas_player_bookmarked) == 0:
            ...
            m 3rksdlb "...Что ж, похоже, ты уже узнал об одной из возможностей, о которой я собиралась тебе рассказать, {do_giggle}а-ха-ха!"
...
label mas_islands_reset:
    ...
    menu:
        ...
        "Давай сделаем это!":
            ...
            m 3kuu "...И мне будет чем заняться, когда ты отсутствуешь. {do_giggle}Э-хе-хе~"
...
Всё, вперёд, пацан! Продолжай и дальше радовать Монику! #Откуда тут пацан взялось?
