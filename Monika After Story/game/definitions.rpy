...
init -1 python:
    ...
    def delete_auto_saves():
        for autosave in renpy.list_saved_games(regexp='auto', fast=True):
            renpy.unlink_save(autosave)
            
    def delete_reload_saves():
        for reloadsave in renpy.list_saved_games(regexp='reload', fast=True):
            renpy.unlink_save(reloadsave)
...
init 21 python:
    def mas_get_player_nickname(capitalize=False, exclude_names=[], _default=None, regex_replace_with_nullstr=None):
        ...
        nickname_pool = persistent._mas_player_nicknames + [player_abb]
        ...
        if persistent.gender == "F":
            selected_nickname = (
                selected_nickname.replace("love", "любимая")
                .replace("honey", "милая")
                .replace("darling", "дорогая")
                )
        else:
            selected_nickname = (
                selected_nickname.replace("love", "любимый")
                .replace("honey", "милый")
...
default monika_name = persistent._mas_monika_nickname
...
define lower_letters_only = " abcdefghijklmnopqrstuvwxyzабвгдеёжзийклмнопрстуфхчшщцьыъэюя"
...
define name_characters_only = " abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZабвгдеёжзийклмнопрстуфхчшщцьыъэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЧШЩЦЬЫЪЭЮЯ-_'"
