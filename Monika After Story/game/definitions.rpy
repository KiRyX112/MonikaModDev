...
python early:
    ...
    def __mas_periodic_override(self):
        ...
        if self.loop and not self.queue:
            for i in self.loop:
                if topq is not None:
                    newq = renpy.audio.audio.QueueEntry(i, 0, topq.tight, True, 1.0)
                else:
                    newq = renpy.audio.audio.QueueEntry(i, 0, False, True, 1.0)
...
define lower_letters_only = " абвгдеёжзийклмнопрстуфхцчшщъыьэюя"
