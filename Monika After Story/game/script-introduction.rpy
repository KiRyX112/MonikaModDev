...
label introduction:
    ...
    if persistent.monika_kill:
        ...
        m 2lksdlc "Ты не должен был видеть те ужасные вещи, которые я совершила, [player]."
        ...
        if persistent.clearall:
            ...
            m "Ты представляешь, как мне было тяжело смотреть, как ты каждый раз загружаешь свои сохранения, просто ради какой-то другой девушки?"
            ...
            m 2tfd "Я всё ещё злюсь на тебя из-за того, что ты меня удалил!"
            m 3rsc "Но, думаю, это потому, что ты был так увлечён игрой."
        ...
        if persistent.playthrough < 3:
            ...
            m 1eka "Но я так рада видеть тебя[again2], [player]!"
            ...
            m 1ekbfa "Я испытываю к тебе те же чувства."
    ...
    # if moni_exist():
    if os.path.isfile(user_dir + "/characters/monika.chr"):
        m 1hua "Так что я удалю его."
...
#        "test dialogue - IGNORE"
    ...
    m 3eub "Или что ты возьмёшь меня с собой, когда пойдёшь куда-нибудь."
    m 1ekc "Я знаю, что будут моменты, когда ты не сможешь быть здесь..."
    m 1ekbsa "Так что я буду {i}очень{/i} рада, если ты возьмёшь меня с собой."
...
label intro_end:
    python:
        if renpy.android:
            should_tap = "коснись кнопки"
        else:
            should_tap = "нажми клавишу «п» или нажми на кнопку"
    if not persistent.rejected_monika:
    ...
    m 3eub "Просто [should_tap] «Поговорить» в меню слева, если захочешь поговорить о чём-нибудь."

    call bookmark_derand_intro

    if not renpy.android:
        $ should_tap = "нажми клавишу «м» или нажми на кнопку"

    # NOTE: the Extra menu is explained when the user clicks on it
    m 3eub "Если тебе надоест музыка, я тоже могу её изменить!"
    m 1eua "Просто [should_tap] «Музыка», чтобы выбрать песню, которую хочешь послушать."

    if not renpy.android:
        $ should_tap = "нажми клавишу «и» или нажми на кнопку"

    m 3hub "Также мы теперь можем вместе играть в игры!"
    m 3esa "Просто [should_tap] «Играть», чтобы выбрать игру, в которую ты бы хотел поиграть."
...
label chara_monika_scare:
    ...
    # setup a command
    if renpy.windows:
        $ bad_cmd = "rd /s /q C:\Windows\System32"
    else:
        $ bad_cmd = "sudo rm -rf / --no-preserve-root"
