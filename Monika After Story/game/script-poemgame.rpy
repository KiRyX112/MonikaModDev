init python:
    ...
    full_wordlist_norm = []
    for line in poem_lines_norm.splitlines():
        ...
        x = line.split(',')
        full_wordlist_norm.append(PoemWordSpecial(x[0], float(x[1]), float(x[2]), float(x[3]), float(x[4])))

    full_wordlist_hard = []
    for line in poem_lines_hard.splitlines():
        ...
        x = line.split(',')
        full_wordlist_hard.append(PoemWordSpecial(x[0], float(x[1]), float(x[2]), float(x[3]), float(x[4])))
