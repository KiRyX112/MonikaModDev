init -1 python:
    tt_awc_desc = (
        "Нажмите, чтобы погода у Моники совпадала с вашей."
    )

    tt_ast_desc = (
        "Нажмите, чтобы синхронизировать время заката и рассвета у Моники с вашим."
    )
    ...
    vbox:
        ...
        hbox:
            if bool(persistent._awc_API_key):
                ...
                textbutton _("Менять время суток"):
                ...
                text "Добавьте действительный API-ключ.": #You can create one {a=https://openweathermap.org/api}{i}{u}here{/u}{/i}{/a}.":
                    xalign 1.0 yalign 0.0
                    style "main_menu_version"

    if bool(persistent._awc_API_key):
        text "API-ключ действителен":
            xalign 1.0 yalign 0.0
            xoffset -10
            style "main_menu_version"
...
# Разобраться в сути работы с multimokia ** А что такое суть? 0_о
