...
label monika_sethair_ponytail:
    ...
    if (
        ...
        if store.mas_globals.in_idle_mode or (mas_canCheckActiveWindow() and not mas_isFocused()):
            if store.mas_globals.ahc_run_after_date:
                m 1eua "Я просто хочу устроиться поудобнее.{w=0.5}.{w=0.5}.{w=1}{nw}"
            else:
                m 3eua "Мне нужно подготовиться к сегодняшнему дню.{w=0.5}.{w=0.5}.{w=1}{nw}"

        else:
            if store.mas_globals.ahc_run_after_date:
                m 1eua "Секундочку, [mas_get_player_nickname()], я устроюсь немного покомфортнее.{w=0.5}.{w=0.5}.{nw}"
            else:
                m 1eua "Подожди немного, [mas_get_player_nickname()]."
                m 3eua "Я подготовлюсь к сегодняшнему дню.{w=0.5}.{w=0.5}.{nw}"
        ...
        if store.mas_globals.in_idle_mode or (mas_canCheckActiveWindow() and not mas_isFocused()):
            ...
            m 3hub "Готово!"
            m 1eua "Если хочешь чтобы я сменила причёску, просто попроси, хорошо?"
    return
...
label monika_sethair_down:
    ...
    if (
        ...
        if store.mas_globals.in_idle_mode or (mas_canCheckActiveWindow() and not mas_isFocused()):
            ...
            m 5hua "Если хочешь что-нибудь поменять, просто попроси~"
...
label monika_setoutfit_pjs:
    ...
    if store.mas_globals.in_idle_mode or (mas_canCheckActiveWindow() and not mas_isFocused()):
        m 1eua "Сейчас, я надену свою пижаму.{w=0.5}.{w=0.5}.{w=1}{nw}"
    else:
        m 1eua "Одну минутку, [mas_get_player_nickname()], я надену свою пижаму.{w=0.5}.{w=0.5}.{nw}"
