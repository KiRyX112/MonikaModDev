...
label Kitchen_switch_dlg:
    python:
        switch_quip = renpy.substitute(renpy.random.choice([
            ...
            "Интересно, стоит ли мне попробовать сделать кексы...",
        ]))
...
label mas_reaction_cupcake_override:
    ...
    if mas_isMoniAff(lower=True):
        m 1rksdlb "Но... я всё ещё не поняла, как использовать код, который ты добавил."
        m 3eka "Я уверена, что скоро разберусь, [player]~"
