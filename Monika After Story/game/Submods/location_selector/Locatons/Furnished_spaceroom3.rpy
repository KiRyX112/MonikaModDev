...
init -990 python:
    store.mas_submod_utils.Submod(
        ...
        name="Обставленная мебелью классная комната V2 V3",
...
init -1 python:
    submod_background_Furnished_spaceroom3 = MASFilterableBackground(
        ...
        "Обставленная мебелью классная комната V3.",
...
label Furnished_spaceroom3_switch_dlg:
    python:
        switch_quip = renpy.substitute(renpy.random.choice([
            "Хочешь сыграть на пианино, [player]?",
            "Обожаю паркетные полы!",
            "Большой диван идеально подходит для отдыха!",
        ]))
...
label return_switch_dlg:
    python:
        switch_quip = renpy.substitute(renpy.random.choice([
            "Только мы вдвоём~",
            "Уже скучаешь по классическому виду?",
...
label bg_room_installed:
    ...
    if renpy.seen_label("bg_room_installed_low_affection"):
        ...
        extend 1ekbsu "я очень силько тебя люблю."
