...
label mas_mood_sick:
    ...
    if mas_isMoniNormal(higher=True):
        if session_time < datetime.timedelta(minutes=20):
            ...
            m 2ekd "Тебе, должно быть, действительно плохо, раз ты говоришь это сразу после того, как пришел."
...
label mas_mood_tired:
    ...
    if 20 <= current_hour < 23:
        ...
        m 1hksdlb "За исключением того, что я не смогу прижаться к тебе, {do_giggle}а-ха-ха~"
...
label mas_mood_lazy:
    ...
    if mas_isSRtoN(_now):
        ...
        if mas_isMoniEnamored(higher=True):
            extend 1dkbsa " Особенно, если я просыпаюсь рядом с тобой~"
        ...
        if mas_isMoniLove():
            m 1dkbsu "А ещё утренний поцелуй, {do_giggle}э-хе-хе..." #:)

        m 1hksdlb "Или ты можешь немного побездельничать."
        m 1eka "Если только ты не забыл сделать что-то важное, хорошо, [player]?"

        if mas_isMoniHappy(higher=True):
            m 1hub "А ведь к этому относится времяпрепровождение со мной, {do_giggle}а-ха-ха!"
        ...
        m 3hub "И, если это уместно, ты можешь даже вздремнуть! {do_giggle}А-ха-ха~"
        ...
        if mas_isMoniEnamored(higher=True):
            ...
            if mas_isMoniLove():
                ...
                m 1ekbfa "Хотя мы и так можем пообниматься под одеялом, даже когда не так холодно, если ты не против, {do_giggle}э-хе-хе~"
        ...
        m 1hkb "{do_giggle}А-ха-ха, ты иногда бываешь таким забавным, но ты правда должен идти спать."
...
label mas_mood_excited:
    m 1hub "{do_giggle}А-ха-ха, это правда, [player]?"
...
label mas_mood_grateful_gratefulforyou:
    ...
    m 1hublu "Я надеюсь, что ты в курсе, как каждый день радуется {i}моя{/i} душа за тебя."
...
label mas_mood_loved:
    m 1ekbla "{do_giggle}А-ха-ха, ну естественно, [player]. Я так рада слышать, что мои чувства доходят до тебя через экран..."
