...
label mas_piano_songchoice:
    ...
    $ MAS.MonikaElastic()
    m 1eua "Тогда сыграй со мной, [player_abb]~"
...
label mas_piano_result_none:
    ...
    $ MAS.MonikaElastic()
    m 1eka "Это такое наслаждение для меня слушать, как ты играешь."
    $ MAS.MonikaElastic()
    m 1hua "Но обещай, что в следующий раз сыграешь вместе со мной?"
    return
...
label mas_piano_def_fail:
    ...
    m 1lksdla "..."
    $ MAS.MonikaElastic()
    m 1lksdlb "Ты сделал[mas_gender_none] всё возможное, [player]..."
    return
...
label mas_piano_hb_win:
    ...
    if mas_isMonikaBirthday():
        if mas_isplayer_bday():
            ...
            $ MAS.MonikaElastic()
            m 1hua "Спасибо, что сыграл[mas_gender_none] песню в честь моего дня рождения, [player]."
            $ MAS.MonikaElastic()
...
label mas_piano_hb_fail:
    if mas_isMonikaBirthday():
        if mas_isMoniUpset(lower=True):
            ...
            $ MAS.MonikaElastic()
            m 3hub "Это ты здорово придумал[mas_gender_none], попытаться сыграть эту песню в честь нашего дня рождения!"
            $ MAS.MonikaElastic()
        ...
        m 1lksdla "..."
        $ MAS.MonikaElastic()
        m 1lksdlb "Ты очень старал[mas_gender_sya], [player]..."
        $ MAS.MonikaElastic()
...
label mas_piano_hb_fc:
    ...
    if mas_isMonikaBirthday():
        if mas_isplayer_bday():
            ...
            $ MAS.MonikaElastic(voice="monika_giggle")
            m 1rusdlb "А-ха-ха! Было как-то странно то, что ты сам[mas_gender_none] пел[mas_gender_none] песню про день рождения..."
            $ MAS.MonikaElastic()
            m 1hub "Но ты проделал[mas_gender_none] замечательную работу, играя её!"
            $ MAS.MonikaElastic()
...
label mas_piano_hb_prac:
    if mas_isMonikaBirthday():
        if mas_isplayer_bday():
            ...
            $ MAS.MonikaElastic()
            m 1eua "Спасибо, что пытал[mas_gender_sya] сыграть эту песню в честь моего дня рождения!"
            $ MAS.MonikaElastic()
...
label mas_piano_yr_win:
    ...
    m "Но..."
    $ MAS.MonikaElastic()
    m 1lksdlb "Ты мог[mas_gender_g] бы сыграть лучше с небольшой практикой..."
    $ MAS.MonikaElastic(voice="monika_giggle")
    m 1hksdlb "Э-хе-хе~"
    return
...
label mas_piano_yr_fail:
    ...
    m 1lksdlc "..."
    $ MAS.MonikaElastic()
    m 1eka "Всё в порядке, [player_abb]."
    $ MAS.MonikaElastic()
    m 1hua "По крайней мере, ты старал[mas_gender_sya]."
    return
...
label mas_piano_dpco_win:
    m 2dsc "Не могу поверить, что ты это сделал[mas_gender_none]."
...
label mas_piano_dpco_fail:
    m 1lksdla "Я думаю, это нормально — не учиться этому..."
    return
...
init 800 python in mas_piano_keys:
    ...
    _pnm_dpco_v1l1 = PianoNoteMatch(
        renpy.text.text.Text(
            "Да, ты знаешь, что я провела некоторое время, глядя на тебя. (исп.)",
    ...
    def getSongChoices():
        ...
        return song_list, ("Не важно", "None", False, False, 10)
