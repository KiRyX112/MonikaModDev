...
label mas_piano_hb_win:
    ...
    if mas_isMonikaBirthday_dt(extend_by=1):
        ...
        m 3hub "{do_giggle}А-ха-ха! Это было очень даже замечательно, [player]!"
...
label mas_piano_hb_fc:
    ...
    if mas_isMonikaBirthday_dt(extend_by=1):
        ...
        $ persistent._mas_bday_no_recognize = False
        if mas_isplayer_bday():
            m 3hub "{do_giggle}А-ха-ха! Это было {i}очень{/i} здорово!"
            ...
            m 1rusdlb "{do_giggle}А-ха-ха! Было как-то странно то, что ты сам пел песню про день рождения..."
        ...
        m 3hub "{do_giggle}А-ха-ха! Это было очень здорово, [player]!"
        m 1hua "Всегда приятно подпевать под музыку на досуге!"
    else:
...
label mas_piano_hb_prac:
    if mas_isMonikaBirthday():
        ...
        m 1eksdla "{do_giggle}Э-хе-хе, пытаешься разучить песню про день рождения к своему дню рождения, [player]?"
        m 3hua "Продолжай стараться, я знаю, у тебя получится!"
    return
