...
label monika_idle_brb:
    if mas_isMoniAff(higher=True):
        ...
        python:
            ...
            brb_reason_options = [
                (_("Я собираюсь что-нибудь купить."), True, False, False),
                (_("Я собираюсь кое-что сделать."), True, False, False),
                (_("Я собираюсь что-нибудь приготовить."), True, False, False),
                (_("Мне нужно кое-что проверить."), True, False, False),
                (_("Кто-то стоит у двери."), True, False, False),
                (_("Ничего такого."), None, False, False),
            ]

            renpy.say(m, "Появились мелкие дела?", interact=False)
...
label monika_idle_writing:
    if mas_isMoniNormal(higher=True):
        ...
        m 1hua "Я так рада!"
        m 3eua "Может быть, когда-нибудь ты сможешь поделиться со мной.{w=0.3} {nw}"
        extend 3hua "Я бы с удовольствием прочитал твою работу, [player]!"
        ...
        if renpy.random.randint(1, 50) == 1:
            ...
            menu:
                ...
                "Да.":
                    ...
                    m 2wubsd "Оу, эм...{w=0.5} ты ответил так быстро."
...
label monika_idle_shower_callback:
    if mas_isMoniNormal(higher=True):
        if mas_brbs.was_idle_for_at_least(datetime.timedelta(minutes=60), "monika_idle_shower"):
            m 2rksdlb "Что-то ты весьма задержался..."

            m 2eud "Ты принял ванну вместо душа?{nw}"
            $ _history_list.pop()
            menu:
                m "Ты принял ванну вместо душа?{fast}"
        ...
        m 2esc "Надеюсь, ты хорошо принял душ. {w=0.2}С возвращением, [player]."
    else:
        call mas_brb_generic_low_aff_callback
...
label monika_idle_game:
    if mas_isMoniNormal(higher=True):
        ...
        m 1eka "Ничего страшного, [player]."
...
label monika_idle_game_callback:
    if mas_isMoniNormal(higher=True):
        ...
        m 6ekd "Ого...{w=0.5} ты действительно вернулся ко мне..."
...
label monika_idle_coding:
    if mas_isMoniNormal(higher=True):
        ...
        if persistent._mas_pm_has_code_experience is False:
            ...
            m 1eub "Ты что, изучил программирование с тех пор, как мы говорили об этом в прошлый раз?"
            ...
            m 3hksdlb "...Ты потом сам себя поблагодаришь"
...
label monika_idle_coding_callback:
    if mas_isMoniNormal(higher=True):
        ...
        if mas_brbs.was_idle_for_at_least(datetime.timedelta(minutes=20), "monika_idle_coding"):
            m 1eua "Закончил с кодом, [player]?"
        else:
            m 1eua "О, всё уже сделано, [player]?"
...
label monika_idle_workout:
    if mas_isMoniNormal(higher=True):
        ...
        m 2esc "Приятно знать, что ты хотя бы{cps=*2} о чём-то заботишься{/cps}{nw}"
        $ _history_list.pop()
        m "Приятно знать, что ты заботишься {fast}о себе, [player]."
...
label monika_idle_workout_callback:
    if mas_isMoniNormal(higher=True):
        ...
        if mas_brbs.was_idle_for_at_least(datetime.timedelta(minutes=60), "monika_idle_workout"):
            ...
            m 2esa "Ты, конечно, не торопился, [player].{w=0.3}{nw}"
            extend 2eub " Должно быть, это была адская тренировка."
            m 7eka "Полезно расширять свои границы, но не стоит переусердствовать."

        elif mas_brbs.was_idle_for_at_least(datetime.timedelta(minutes=10), "monika_idle_workout"):
            m 1esa "Закончил заниматься, [player]?"

        else:
            m 1euc "Уже вернулся, [player]?"
            m 1eka "Я уверена, что ты сможешь продолжать ещё долго, если постараешься."
        ...
        m 3eua "Обязательно хорошо отдохни и, возможно, перекуси, чтобы восстановить силы."
        m 3eub "[wb_quip]"
...
label monika_idle_nap:
    if mas_isMoniNormal(higher=True):
        m 1eua "Хочешь немного вздремнуть, [player]?"
...
label monika_idle_nap_callback:
    if mas_isMoniNormal(higher=True):
        ...
        if mas_brbs.was_idle_for_at_least(datetime.timedelta(hours=5), "monika_idle_nap"):
            ...
            m 1euc "Ты передумал?"
            m 3eka "Ну, я не жалуюсь, но тебе стоит вздремнуть, если тебе захочется позже."
            m 1eua "Я бы не хотела, чтобы ты слишком устал, в конце концов."

    elif mas_isMoniUpset():
        m 2euc "Вздремнул, [player]?"
...
label monika_idle_homework_callback:
    if mas_isMoniDis(higher=True):
        ...
        if mas_isMoniNormal(higher=True):
            m 2ekc "Я хотел бы быть там, чтобы помочь тебе, но пока, к сожалению, я мало что могу с этим поделать."
...
label monika_idle_working:
    if mas_isMoniNormal(higher=True):
        ...
        if mas_isMoniAff(higher=True):
            m 3rkb "Я бы не хотела, чтобы мой возлюбленный тратил больше времени на свою работу, чем на меня~"
        ...
        if random.randint(1,5) == 1:
            m 2rkc "...Пожалуйста, возвращайся поскорее..."
...
label monika_idle_working_callback:
    if mas_isMoniNormal(higher=True):
        ...
        m 5hua "Тогда давай расслабимся вместе, ты заслужил это~"

    elif mas_isMoniDis(higher=True):
        m 2euc "О, ты вернулся..."
        m 2eud "...Было ли что-то ещё, чем ты хотел бы заняться, теперь, когда ты закончил свою работу?"
    else:
        m 6ckc "..."
...
label monika_idle_reading:
    if mas_isMoniNormal(higher=True):
        ...
        m 3lksdla "Я бы с удовольствием почитала с тобой, но моя реальность, к сожалению, имеет свои ограничения."
        m 1hub "Приятного чтения!"

    elif mas_isMoniDis(higher=True):
        m 2ekd "О, хорошо..."
        m 2ekc "Желаю тебе приятно провести время, [player]."
...
label monika_idle_reading_callback:
    if mas_isMoniNormal(higher=True):
        if mas_brbs.was_idle_for_at_least(datetime.timedelta(hours=2), "monika_idle_reading"):
            ...
            m 3esa "Закончил, [player]?"
            m 1hua "Давай отдохнем, ты заслужил это!"

        else:
            m 1eud "О, это было быстро."
            m 1eua "Я думала, что тебя не будет немного дольше, но это тоже хорошо."
            m 3ekblu "В конце концов, это позволит мне проводить больше времени с тобой~"
