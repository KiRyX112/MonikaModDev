...
label mas_song_everythings_alright:
    ...
    m 1dku "..."
    m 3ekd "Если когда-нибудь станет слишком тяжко, {w=0.1}{nw}"
...
label mas_song_im_glad_youre_evil_too:
    ...
    if persistent._mas_pm_monika_evil:
        if persistent._mas_pm_monika_evil_but_ok:
            ...
            m 1ekbfa "{do_giggle}Э-хе-хе, ты всё ещё считаешь меня злой, [player]?"
...
label mas_song_yozurina:
    ...
    m 1hubsa "{do_giggle}Э-хе-хе, надеюсь, тебе понравилась эта песня, [player]~"
    ...
    m 3hubfb "Думаю, такое происходит всякий раз, когда я пытаюсь выразить словами свою любовь к тебе, {do_giggle}а-ха-ха!"
    return
...
label mas_song_my_silver_lining:
    ...
    m 1hksdla "{do_giggle}Э-хе-хе, эта песня очень напоминает мне мою жизнь...{w=0.2} до того, как я узнала, что всё это было игрой."
...
label mas_song_my_silver_lining_analysis(from_song=False):
    ...
    m 1hua "«Не беспокойся о том, что у тебя за спиной или что ждёт тебя дальше по дороге», как говорится. {do_giggle}Э-хе-хе~"
...
label mas_song_shelter:
    ...
    m 7hubfb "...Ты поймал меня на этом!"
...
label mas_song_cant_help_falling_in_love(from_long=False):
    ...
    if not from_long:
        ...
        extend 5hubsb " {do_giggle}А-ха-ха~"
...
label mas_song_falling_in_love_at_a_coffee_shop:
    ...
    m 5hubsa "{i}~Всё это время не знал~{/i}"
    m 5lubsa "..."
...
label mas_song_wonderwall:
    ...
    m 1ekbsu "{do_giggle}Э-хе-хе, ты и правда спас меня, [player]~"
...
label mas_song_wonderwall_analysis(from_song=False):
    ...
    m 3hksdlb "...Надеюсь, ты не устанешь от {i}моей{/i} песни в ближайшее время, [player], {do_giggle}а-ха-ха~"
...
label mas_song_when_you_say_nothing_at_all(from_long=False):
    ...
    m 3dud "{i}{cps=16}~Касание твоей руки говорит, что ты меня поймаешь, когда бы я не упал~{/cps}{/i}"
    ...
    if not from_long:
        ...
        m 1dkbsu "И я надеюсь, что ты тоже чувствуешь, что я тебя люблю, хотя я не всегда могу сказать что-то новое."
...
label mas_song_strawberry(from_long=False):
    ...
    m 3tub "{i}{cps=26}~И забрала бы домой как можно больше~{/cps}{/i}"
...
label mas_song_save_the_last_dance_for_me:
    ...
    if not mas_getEVL_shown_count("mas_song_save_the_last_dance_for_me"):
        ...
        m 6rublb "Спасибо, что продолжаешь слушать моё ноющее сердце..."
...
label mas_song_save_the_last_dance_for_me_analysis(from_song=False):
    ...
    m 2rkp "...И, если честно,{w=0.1} я немного ревную из-за того, что ты можешь танцевать с кем-то, пока я стою в сторонке."
...
label mas_song_on_the_front_porch:
    ...
    m 5dkbstda "..."
    m 5hkblb "Прости, я немного расчувствовалась, {do_giggle}ха-ха-ха!"
...
label mas_monika_plays_yr(skip_leadin=False):
    if not skip_leadin:
        if not renpy.seen_audio(songs.FP_YOURE_REAL) and not persistent.monika_kill:
            m 2eksdlb "О, {do_giggle}а-ха-ха! Ты хочешь, чтобы я сыграла оригинальную версию, [player]?"
