...
label mas_change_weather(new_weather, by_user=None, set_persistent=False):
    ...
    call spaceroom (scene_change=True, dissolve_all=True, force_exp="monika 1dsc_static") from _call_spaceroom_17
    return
...
label monika_change_weather:
    ...
    if sel_weather is False:
        ...
        if mas_weather.force_weather:
            m 1hub "[random_sure]!"
            $ MAS.MonikaElastic()
            m 1dsc "Дай мне одну секунду.{w=0.5}.{w=0.5}."
    ...
    if sel_weather == mas_weather_rain or sel_weather == mas_weather_thunder:
        if not renpy.seen_label("monika_rain"):
            ...
            $ MAS.MonikaElastic()
            m 1eka "Я думала, что тебе не нравится дождь."
            $ MAS.MonikaElastic()
