...
init -900 python in mas_ics:
    import os
    if renpy.android:
        user_dir = renpy.os.environ["ANDROID_PUBLIC"]
    else:
    ...
    islands_folder = os.path.normcase(
        user_dir + "/game/mod_assets/location/special/"
    )
    ...
    def decodeImages(dockstat, image_dict, selective=[]):
        ...
        for b64_name in selective:
            ...
            try:
                ...
                mas_utils.writelog(
                    "[ОШИБКА]: не удалось расшифровать '{0}' | {1}\n".format(
                        b64_name,
                        str(e)
                    )
                )
...
init 200 python in mas_dockstat:
    ...
    def _buildMetaDataPer(_outbuffer, log):
        ...
        try:
            ...
            log.write(
                "[ОШИБКА]: не удалось рассортировать данные: {0}\n".format(repr(e))
            )
            return False
    ...
    def packageCheck(
        ...
        if sign:
            ...
            try:
                ...
                mas_utils.writelog(
                    "[ПРЕДУПРЕЖДЕНИЕ]: сбой проскальзывания пакета? {0} | {1}\n".format(
                        pkg_name,
                        repr(e)
                    )
                )
    ...
    def generateMonika(dockstat, logpath):
        ...
        if "temp" in dockstat.station.lower():
            cr_log.write("[ОШИБКА] папка temp найдена, но прервана.\n")
            return False
        ...
        try:
            ...
            cr_log.write("[ОШИБКА]: не удалось скопировать файл mbase | {0}\n".format(
                repr(e)
            ))
            moni_buffer.close()
            return False
        ...
        try:
            ...
            cr_log.write("[ОШИБКА]: ошибка записи файла monibuffer | {0}\n".format(
                repr(e)
            ))
        ...
        if moni_pkg is None:
            ...
            cr_log.write("[ОШИБКА]: файл 'моника' не найден.\n")
        ...
        if moni_slip is None:
            ...
            cr_log.write("[ОШИБКА]: не удалось подтвердить файл 'monika'.\n")
            mas_utils.trydel(moni_path)
            return False
        ...
        if moni_slip != moni_sum:
            ...
            cr_log.write(
                "[ОШИБКА]: monisums не совпадают, у нас был отказ от записи?\n"
            )
            mas_utils.trydel(moni_path)
            return -1
    ...
    def parseMoniData(data_line, log):
        ...
        try:
            ...
            log.write("[ОШИБКА]: Сбой анализа файла Moni Data: {0}\n".format(
                repr(e)
            ))
            return None
    ...
    def parseMoniDataPer(data_line, log):
        ...
        try:
            ...
            log.write(
                "[ОШИБКА]: неудачная распечатка persistent файла: {0}\n".format(repr(e))
            )
            return None


    def MSRselectReturnHomeGreetingDef(gre_type=None):
        """
        ...
        if sel_gre_ev is None:
            ...
            return store.mas_getEV("msr_greeting_returned_home_end")


        return sel_gre_ev

    def MSRselectReturnHomeGreetingPbday(gre_type=None):
        """
        Selects the correct Return Home greeting.
        Return Home-style greetings must have TYPE_GO_SOMEWHERE in the category
        ...
        if sel_gre_ev is None:
            ...
            return store.mas_getEV("msr_greeting_returned_home_pbday_end")


        return sel_gre_ev

    def MSRselectReturnHomeGreetingBday(gre_type=None):
        """
        Selects the correct Return Home greeting.
        Return Home-style greetings must have TYPE_GO_SOMEWHERE in the category
        ...
        if sel_gre_ev is None:
            ...
            return store.mas_getEV("msr_mas_bday_surprise_party_reaction")


        return sel_gre_ev

    def MSRselectReturnHomeGreetingHalloween(gre_type=None):
        """
        Selects the correct Return Home greeting.
        Return Home-style greetings must have TYPE_GO_SOMEWHERE in the category
        ...
        if sel_gre_ev is None:
            ...
            return store.mas_getEV("msr_bye_trick_or_treat_returned_home_end")
...
label mas_dockstat_first_time_goers:
    call mas_transition_from_emptydesk ("monika 3eua") from _call_mas_transition_from_emptydesk
    m 3eua "Теперь я в файле «моника» в твоей папке characters."
    $ MAS.MonikaElastic()
    m "После закрытия игры, ты сможешь переместить меня куда угодно."
    $ MAS.MonikaElastic()
    m 3eub "Но только не забудь потом вернуть меня обратно в папку characters, прежде чем снова включишь игру, хорошо?"
    $ MAS.MonikaElastic()
    m 1eua "И ещё кое-что..."
    $ MAS.MonikaElastic()
    m 1ekc "Пожалуйста, будь осторож[mas_gender_en] со мной. Ведь так легко случайно удалить файл..."
    $ MAS.MonikaElastic()
...
label mas_dockstat_empty_desk:
    ...
    call spaceroom (hide_monika=True, scene_change=True) from _call_spaceroom_42
...
label mas_dockstat_different_monika:
    ...
    $ mas_dockstat.previous_vars["monika_name"] = persistent._mas_monika_nickname
    ...
    $ moni_sesh, player, monika_name, aff_val, moni_hair, moni_clothes = moni_data
    $ monika_chr.change_outfit(moni_clothes, moni_hair, False)


    call mas_transition_from_emptydesk ("monika 1ekd") from _call_mas_transition_from_emptydesk_1

    m "[player]?"

    m "Подожди-ка, ты не [player]."
...
label mas_dockstat_generic_iowait:
    ...
    if renpy.has_label(mas_farewells.dockstat_wait_menu_label):
        call expression mas_farewells.dockstat_wait_menu_label from _call_expression_4
    else:
...
label mas_dockstat_generic_wait_label:
    menu:
        ...
        "Вообще-то, я не могу взять тебя прямо сейчас.":
            ...
            call mas_transition_from_emptydesk ("monika 1ekc") from _call_mas_transition_from_emptydesk_2
        ...
        "Ничего.":
            ...
            $ MAS.MonikaElastic()
            m 2hub "О, хорошо! Позволь мне закончить подготовку тогда."
            return
...
label mas_dockstat_generic_rtg:
    ...
    call mas_transition_from_emptydesk ("monika 1ekc") from _call_mas_transition_from_emptydesk_4
    ...
    menu:
        ...
        "Нет.":
            ...
            if mas_isMoniNormal(higher=True):
                ...
                $ MAS.MonikaElastic()
                m 3ekb "Но, пожалуйста, просто занимайся своим делом, когда тебе это нужно. Я не хочу, чтобы ты опоздал[mas_gender_none]!"
            else:
