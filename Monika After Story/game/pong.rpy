    ...
    python:
        ...
        class PongDisplayable(renpy.Displayable):
            ...
            def __init__(self):
                ...
                self.ctb = Text(_("Нажми, чтобы начать игру!"), size=36)
...
label mas_pong_dlg_winner:
    ...
    if monika_asks_to_go_easy and ball_paddle_bounces == 1:
        ...
        m 1hksdla "Я действительно просила тебя быть помягче со мной, но я вовсе не это имела ввиду..."
        m 3eka "Но я очень ценю этот жест~"
    ...
    #The player fails to hit the first ball
        ...
        if instant_loss_streak_counter == 1:
            m 2rksdlb "{do_giggle}А-ха-ха, как жаль..."

        #Twice
        elif instant_loss_streak_counter == 2:
            m 2rksdlc "[player],{w=0.1} ты снова проиграл..."
        ...
        #Thrice
            ...
            if persistent._mas_pm_ever_let_monika_win_on_purpose:
                $ menu_response = _("Ты опять поддаёшься мне?")
            else:
                $ menu_response = _("Ты поддаёшься мне?")
            ...
            menu:
                ...
                "Может быть...":
                    ...
                    if persistent._mas_pm_ever_let_monika_win_on_purpose:
                        m 5eua "Мне нравится наблюдать за твоей победой точно так же, как тебе нравится наблюдать за моей~"
                    ...
                    if persistent._mas_pm_ever_let_monika_win_on_purpose:
                        ...
                        menu:
                            ...
                            "Нет":
                                ...
                                m 1rfu "[player]!"
                                m 2hksdlb "Перестань дразниться!"
                                $ player_lets_monika_win_on_purpose = True
        ...
        #Any more times
            ...
            if player_lets_monika_win_on_purpose:
                m 2tku "Тебе не надоело поддаваться мне, [player]?"
            else:
                m 1rsc "..."
        ...
        if persistent._mas_pong_difficulty_change_next_game_date == datetime.date.today():
            ...
            extend 2tfb "Я сказала, что в следующем матче победа будет за мной."


    elif powerup_value_this_game == PONG_DIFFICULTY_POWERDOWN:
        m 1rksdla "Ах..."
        m 3hksdlb "Попробуй снова, [player]!"
        ...
        m 2eka "Ну же, [player]! Я знаю, что ты можешь победить..."
        m 3hub "Постарайся!"
        ...
        m 2hub "Теперь моя очередь сиять~"
        ...
        if pong_monika_last_response_id == PONG_MONIKA_RESPONSE_WIN_LONG_GAME:
            m 3eub "Играть против тебя нелегко, [player]."
            m 1hub "Продолжай в том же духе, ты сможешь победить меня!"
        else:
            m 3hub "Отлично, [player]!"
            m 1tfu "Но и я не промах,{w=0.1} {nw}"
        ...
        if pong_monika_last_response_id == PONG_MONIKA_RESPONSE_WIN_SHORT_GAME:
            ...
            m 4huu "{do_giggle}Э-хе-хе,{w=0.1} {nw}"
        ...
        if pong_monika_last_response_id == PONG_MONIKA_RESPONSE_WIN_TRICKSHOT:
            ...
            m 3hksdlb "Я не хотела, чтобы он так сильно прыгал..."
        ...
        if pong_difficulty_before <= 5:
            if pong_monika_last_response_id == PONG_MONIKA_RESPONSE_WIN_EASY_GAME:
                ...
                m 3hub "Продолжай в том же духе! Я знаю, что ты вот-вот одолеешь меня!"
            ...
            if pong_monika_last_response_id == PONG_MONIKA_RESPONSE_WIN_HARD_GAME:
                m 1hub "{do_giggle}А-ха-ха!"
                m 2tsb "Я слишком хорошо играю?"
                m 1tsu "Шучу, [player]."
                m 3hub "Ты очень хорошо играешь!"
            else:
                if loss_streak_counter > 1:
                    m 1hub "Я снова победила~"
                else:
                    m 1huu "Победа за мной~"
            ...
            if pong_monika_last_response_id == PONG_MONIKA_RESPONSE_WIN_EXPERT_GAME:
                m 2tub "Как приятно побеждать!"
                m 2hub "Не волнуйся, ты вот-вот одолеешь меня, я уверена~"
            else:
                if loss_streak_counter > 1:
                    m 2eub "Ещё один раунд за мной!"
                else:
                    m 2eub "Я победила!"
...
label mas_pong_dlg_sorry_assuming:
    ...
    menu:
        ...
        "Да.":
            ...
            m 1eka "Отлично, [player].{w=0.3} {nw}"
            extend 1hua "Было очень весело! Спасибо, что поиграл в Понг со мной!"
            m 1eua "Дай знать, когда будешь готов сыграть снова."
        ...
        "Нет.":
            ...
            m 1hub "Постарайся, совсем скоро ты победишь меня!"
    return
...
label mas_pong_dlg_loser:
    ...
    if lose_on_purpose:
        ...
        if pong_monika_last_response_id == PONG_MONIKA_RESPONSE_LOSE_WITHOUT_HITTING_BALL:
            m "Может, мне надо больше стараться..."
        else:
            m "Возможно я была слишком медлительной..."
        ...
        m 2tsu "Играем серьёзно, да?~"
        m 2tfu "Покажи, на что способен, [player]!"


    elif loss_streak_counter_before >= 3:
        m 4eub "Поздравляю, [player]!{w=0.3} {nw}"
        extend 2hub "Я знала, что ты победишь после достаточной практики!"
        m 4eua "Помни: упорными тренировкамии ты сможешь достичь всего, к чему стремишься!"
        ...
        m 1hua "Я рада, что ты победил, [player]."


    elif pong_angle_last_shot >= 0.9 or pong_angle_last_shot <= -0.9:
        if pong_monika_last_response_id == PONG_MONIKA_RESPONSE_LOSE_TRICKSHOT:
            m 2wuo "[player]!"
            m 2hksdlb "Я никак не могла попасть!"
        else:
            m 2wuo "Ух ты, отличный удар!"
        ...
        m 2wuo "Вау, [player]..."
        m 2wud "Ты победил три раза подряд..."


        if pong_difficulty_before <= 5:
            m 2tsu "Может, пора наращивать темп?~"
    ...
    #Monika loses a long game
        ...
        if pong_monika_last_response_id == PONG_MONIKA_RESPONSE_LOSE_LONG_GAME:
            m 2wuo "Ух ты,{w=0.1} Я не успеваю за тобой!"
        else:
            m 2hub "Поразительно, [player]!"
            m 4eub "Ты невероятно хорош!"
    ...
    #Monika loses a short game
        ...
        if pong_monika_last_response_id == PONG_MONIKA_RESPONSE_LOSE_SHORT_GAME:
            ...
            m 3eksdla "Думаю, мне стоит больше стараться..."
        else:
            m 1rusdlb "Не думала, что проиграю так быстро."
    ...
    #Monika loses a game
        ...
        #Medium
            ...
            if pong_monika_last_response_id == PONG_MONIKA_RESPONSE_LOSE_MEDIUM_GAME:
                m 1eua "Приятно наблюдать за твоими победами, [player]."
                m 1hub "Продолжай в том же духе~"
            else:
                if win_streak_counter > 1:
                    m 1hub "Ты снова победил! {w=0.2}Молодец~"
                else:
                    m 1eua "Ты победил! {w=0.2}Неплохо."
        ...
        #Hard
            ...
            if pong_monika_last_response_id == PONG_MONIKA_RESPONSE_LOSE_HARD_GAME:
                m 4hub "Ещё одна победа за тобой!"
                m 4eua "Отлично, [player]."
            else:
                if win_streak_counter > 1:
                    m 2hub "Ты вновь победил! {w=0.2}Поздравляю!"
                else:
                    m 2hua "Победа за тобой! {w=0.2}Мои поздравления!"
        ...
        #Expert
            ...
            if pong_monika_last_response_id == PONG_MONIKA_RESPONSE_LOSE_EXPERT_GAME:
                m 2wuo "Вау,{w=0.1} а ведь я так старалась...{w=0.3}тебя не остановить!"
                m 2tfu "Рано или поздно я всё равно одолею тебя, [player]."
                m 3hub "{do_giggle}А-ха-ха!"
            else:
                if win_streak_counter > 1:
                    m 4hub "Ты снова выиграл! {w=0.2}Отличная работа!"
                else:
                    m 4hub "Ты победил! {w=0.2}Впечатляющие!"
...
label mas_pong_dlg_loser_fast:
    m 1eka "Неплохо, [player]."
    m 3tfu "Но в следующий раз победа будет за мной."
...
label mas_pong_dlg_winner_fast:
    m 1eka "Отлично, [player]. Спасибо за игру."
    m 1hua "Было очень весело! Давай как-нибудь сыграем ещё?"
...
label mas_pong_dlg_loser_end:
    m 1wuo "Вау, а ведь я старалась изо всех сил."
    m 1eua "Ты, должно быть, много тренировался, чтобы достичь такого успеха."
    m 2tuu "Думаю, ты хотел впечатлить меня, [player]."
