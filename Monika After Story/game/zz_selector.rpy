...
init -150 python in mas_selspr:
    ...
    def _rule_bow():
        ...
        return (
            not store.monika_chr.is_wearing_clothes_with_exprop("baked outfit")
            and store.monika_chr.is_wearing_hair_with_exprop("bow")
        )
...
init -100 python in mas_selspr:
    ...
    PROMPT_MAP = {
        ...
        "bow": {
            "_ev": "monika_bow_select",
            "_min-items": 1,
            "_rule": _rule_bow,
            "change": "Могла бы ты изменить свой бантик?",
            "wear": "Могла бы ты надеть свой бантик?",
        },
    }
...
init -10 python in mas_selspr:
    ...
    disable_sel_dlg_quips = {
        ...
        DISB_HAIR_BC_CLOTH: [
            ...
            "Эта прическа на самом деле не сочетается с этим нарядом.",
    ...
    def _validate_group_topics():
        """
        Locks selector topics if there are no_unlocked selectables with the
        appropriate group.
...
label monika_clothes_select:
    ...
    if mas_isMoniEnamored(higher=True) and (not persistent.saveblock):
...
label monika_hair_select:
    ...
    python:
        ...
        mailbox = store.mas_selspr.MASSelectableSpriteMailbox(
            "Какую прическу ты хочешь, чтобы я надела?"
        )
        sel_map = {}
...
label monika_hairclip_select:
    call mas_selector_generic_sidebar_select_acs ("left-hair-clip", idle_dlg="Какую заколку ты хочешь, чтобы я надела?") from _call_mas_selector_generic_sidebar_select_acs
    return
